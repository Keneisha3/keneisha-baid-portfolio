...
/* =====================================================================
   DATA PLAYGROUND — an interactive, simplified take on the Electionomics
   model: drag the economic signals, watch the predicted vote share move.
===================================================================== */
function Slider({ label, value, min, max, step, unit, onChange }) {
  return (
    <label className="block">
      <div className="flex items-baseline justify-between">
        <span className="font-sans text-[13px] font-medium text-[#0D3B66]">{label}</span>
        <span className="font-mono text-[13px] tabular-nums text-[#0D3B66]">
          {value.toFixed(step < 1 ? 1 : 0)}
          <span className="text-[#0D3B66]/50">{unit}</span>
        </span>
      </div>
      <input
        type="range"
        ...
        className="mt-2 h-1.5 w-full cursor-pointer appearance-none rounded-full bg-[#0D3B66]/15 accent-[#0D3B66]"
        aria-label={label}
      />
      <div className="mt-1 flex justify-between font-mono text-[9px] uppercase tracking-[0.15em] text-[#0D3B66]/35">
        <span>{min}{unit}</span>
        <span>{max}{unit}</span>
      </div>
    </label>
...
export function DataPlayground() {
  const [gdp, setGdp] = useState(2.2); // GDP growth, %
  const [approval, setApproval] = useState(46); // incumbent approval, %
  const [unemp, setUnemp] = useState(5.0); // unemployment, %

  // Illustrative linear model — a simplified stand-in for the trained
  // Electionomics regression, purely to make the relationships tangible.
  const raw = 46 + 1.7 * gdp + 0.3 * (approval - 50) - 1.2 * (unemp - 5);
  const share = Math.max(30, Math.min(70, raw));
  const challenger = 100 - share;
  const margin = share - challenger;
  const result =
    Math.abs(margin) < 1.5 ? "Too close to call" : margin > 0 ? "Incumbent wins" : "Challenger wins";
  const resultTone = Math.abs(margin) < 1.5 ? "even" : margin > 0 ? "win" : "loss";

  return (
    <section id="playground" className="relative mx-auto max-w-5xl px-6 py-32 sm:px-12">
      <header className="mb-12">
        <Kicker>try it yourself</Kicker>
        <div className="mt-4">
          <BandHeading>Data Playground</BandHeading>
        </div>
        <p className="mt-5 max-w-xl font-sans text-[15px] leading-relaxed text-[#0D3B66]/70">
          A simplified, playable version of my <span className="font-medium text-[#0D3B66]">Electionomics</span>{" "}
          model — it predicts an incumbent&rsquo;s vote share from a few economic signals. Drag the sliders and
          watch the forecast move.
        </p>
      </header>
      ...
      <div className="grid gap-10 md:grid-cols-2 md:gap-14">
        ...
        <div className="space-y-8">
          <Slider label="GDP growth" value={gdp} min={-3} max={6} step={0.1} unit="%" onChange={setGdp} />
          <Slider label="Incumbent approval" value={approval} min={30} max={70} step={1} unit="%" onChange={setApproval} />
          <Slider label="Unemployment" value={unemp} min={2} max={10} step={0.1} unit="%" onChange={setUnemp} />
        </div>

        {/* the live forecast */}
        <div className="rounded-2xl bg-[#FAF0CA] p-6 sm:p-8">
          <div className="flex items-baseline justify-between">
            <span className="font-mono text-[10px] uppercase tracking-[0.3em] text-[#0D3B66]/60">
              Predicted incumbent share
            </span>
            <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-[#0D3B66]/60">R² 0.78</span>
          </div>

          <div className="mt-2 font-serifDisplay text-6xl font-semibold leading-none text-[#0D3B66]">
            {share.toFixed(1)}<span className="text-3xl align-top">%</span>
          </div>

          {/* split bar with a 50% threshold marker */}
          <div className="relative mt-6 h-9 w-full overflow-hidden rounded-full bg-[#0D3B66]/12">
            <div
              className="h-full rounded-full bg-[#0D3B66] transition-[width] duration-300 ease-out"
              style={{ width: `${share}%` }}
            />
            <div className="absolute inset-y-0 left-1/2 w-px -translate-x-1/2 bg-[#0D3B66]/40" />
            <span className="absolute left-1/2 top-full mt-1 -translate-x-1/2 font-mono text-[9px] uppercase tracking-[0.2em] text-[#0D3B66]/45">
              50% to win
            </span>
          </div>

          <div className="mt-8 flex items-center justify-between">
            <div className="font-sans text-[13px] text-[#0D3B66]/70">
              Incumbent <span className="font-medium text-[#0D3B66]">{share.toFixed(1)}%</span> ·
              Challenger <span className="font-medium text-[#0D3B66]">{challenger.toFixed(1)}%</span>
            </div>
            <span
              className={`inline-flex shrink-0 rounded-full px-3 py-1.5 font-mono text-[11px] font-semibold uppercase tracking-[0.15em] ${
                resultTone === "win"
                  ? "bg-[#0D3B66] text-[#FAF0CA]"
                  : resultTone === "loss"
                  ? "border border-[#0D3B66]/40 text-[#0D3B66]"
                  : "border border-dashed border-[#0D3B66]/50 text-[#0D3B66]/70"
              }`}
            >
              {result}
            </span>
      ...
      <p className="mt-8 max-w-xl font-sans text-[12px] leading-relaxed text-[#0D3B66]/45">
        Illustrative demo — the coefficients here are simplified to keep it playable. The full trained model
        (R² = 0.779) and write-up live in{" "}
        <a href="#projects" className="underline decoration-[#0D3B66]/30 underline-offset-4 hover:text-[#0D3B66]">
          Projects
        </a>
        .
      </p>
